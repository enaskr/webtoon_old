...
if($_GET['keyword'] != null){
	...
	if ( $ends != "END" ) {
		$get_html_contents = file_get_html($base_url."웹툰/작품?sel_sort=latest");
		for($html_c = 0; $html_c < $try_count; $html_c++){
			if(strlen($get_html_contents) > 50000){
				break;
			} else {
		...
		foreach($get_html_contents->find('div.section-item-inner') as $e){
			$loopcnt++;
			if ( $loopcnt > $list_count ) break;
			$term = "";
			$f = str_get_html($e->innertext);
			...
			foreach($f->find('div.section-item-title') as $g){
				$subject = trim(strip_tags($g));
			}
			foreach($f->find('a') as $g){
				$target_link = $g->href;
